...
Build it with `make setup-axe-driver`. Pass a quoted goal and `--udid`, or send one JSON request on standard input. CLI goals default to eight steps; an explicit “Open Calendar app” goal launches Calendar directly, and a dated event with a quoted title derives exact `text` and a visible saved-event requirement. For ambiguous goals, supply `--text` and `--requirement` explicitly. CLI output is human-readable; `--trace PATH` writes full JSON and `--json` prints it. JSON stdin requests remain explicit and machine-readable: include `simulatorUDID`, `instruction`, and `maxSteps`, plus exact `text` and completion evidence where needed. Treat only `completed` as verified. On `done_unverified`, `uncertain`, `stale`, or `uncertain_write`, inspect the trace or returned rows before deciding whether to continue.
