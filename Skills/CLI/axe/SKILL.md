...
Build it with `make setup-axe-driver`. Pass a quoted goal and `--udid`, or send one JSON request on standard input. CLI goals default to eight steps. Supply `--app-bundle-id` to launch an app, `--text` for exact caller-owned input, and `--requirement` or `--expect-label`/`--expect-label-prefix`/`--expect-id`/`--expect-value` for completion evidence; the goal sentence alone does not supply these values. `--expect-label-prefix` checks case-sensitive leading text when an app appends metadata to a title. CLI output is human-readable; `--trace PATH` writes full JSON and `--json` prints it. JSON stdin requests remain explicit and machine-readable: include `simulatorUDID`, `instruction`, and `maxSteps`, plus exact `text` and completion evidence where needed. Treat only `completed` as verified. On `done_unverified`, `uncertain`, `stale`, or `uncertain_write`, inspect the trace or returned rows before deciding whether to continue.
