...
AXe builds IDB from the immutable fork revision configured in `scripts/build.sh`, based on a verified upstream IDB revision. The pinned fork caches the iOS 27 accessibility bootstrap for five seconds, avoiding a new automation session on every read. The build does not apply a local patch queue.
